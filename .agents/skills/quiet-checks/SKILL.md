...
name: quiet-checks
description: |
  Run tests, builds, linters, and installs in their quiet or silent mode so a passing run costs almost no context. Use for choosing the right --quiet, -q, --silent or reporter flag per tool, capping log output, keeping the exit status readable, and re-running only the failure verbosely.
---
