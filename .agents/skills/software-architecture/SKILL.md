...
Map what exists with `starting-dev` first. Never redesign a system you have only read about.

## Workflow
1. Map what exists — entry points, dependencies, ownership, runtime boundaries, change pain — with `starting-dev`. Never redesign a system you have only read about.
...
4. Protect behavior with characterization tests before moving a boundary — `code-review` covers writing them.
...
- Moving a boundary and changing behavior in one step makes the failure unattributable; sequence it with `starting-dev` and `data-engineering`.
- The names the boundaries carry belong to [domain-modeling](references/domain-modeling.md); a module diagram that lives in the repo to `documentation`.
