...
Every plan written by [improve-animations](improve-animations.md) follows this structure. The executor may be a less capable model with zero context and zero taste — the plan must contain everything, exactly. No references to "the audit above" or "the easing we discussed."
...
- Pull every value from [improve-audit.md](improve-audit.md) — never approximate from memory.
