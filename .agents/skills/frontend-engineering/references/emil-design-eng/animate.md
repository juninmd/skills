...
A construction skill. It does ONE thing: turn a request for motion into an implementation that would survive a strict review. It does not audit a codebase (that's [improve-animations](improve-animations.md)), critique a diff (that's [review-animations](review-animations.md)), hunt for places that could animate (that's [find-animation-opportunities](find-animation-opportunities.md)), or build for React Native (that's `mobile-engineering` (animate-expo reference)).
...
If the task needs a *component* rather than an animation — a toast, a drawer, a command menu, a dropdown — stop and invoke `stack-selection` (pick-ui-library reference). Hand-rolling those is how you end up with a `<div>` dropdown and no focus management.
