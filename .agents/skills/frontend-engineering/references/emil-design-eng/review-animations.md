...
For the full rule catalog (easing curves, duration tables, spring config, gestures, clip-path, performance, a11y), see [review-standards.md](review-standards.md). Load it whenever a finding needs a precise value or citation.
...
4. **Sub-300ms UI.** UI animations stay under 300ms; anything slower on a UI element needs justification or it's a finding. Per-element budgets live in [review-standards.md](review-standards.md).
...
Be specific and cite `file:line`. When a value is needed (a curve, a duration, a spring config), pull the exact one from [review-standards.md](review-standards.md) rather than approximating.
