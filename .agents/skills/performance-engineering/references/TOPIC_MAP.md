...
| `real-world-cases.md` | Backend/frontend perf incident case studies (ORM N+1 spike, event-loop stall, unbounded cache leak, false regression from a cold benchmark, silent bundle bloat, instruction-count CI gates, frame-budget streaming, costly CSS selectors) for pattern-matching a symptom |
