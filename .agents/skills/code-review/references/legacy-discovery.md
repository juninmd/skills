...
|---|---|---|
| 1 · Reconnaissance | structure, stack, entry points, config, CI | delegate to `starting-dev`; record its output, never re-derive it |
...
Then fork: rewrite or replatform to `data-engineering`; in-place cleanup to [legacy-refactoring procedure](legacy-refactoring.md).
...
- Diagrams live as text in the repository, never as binary exports — `documentation` owns the mechanics.
...
- [ ] Phase 1 delegated to `starting-dev` and recorded, not re-derived.
