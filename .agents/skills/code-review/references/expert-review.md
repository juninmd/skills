...
## Reference Routing
- Practical review cases: [real-world-cases.md](real-world-cases.md)
- Use [doc-formats.md](doc-formats.md) when reviewing PRDs, specs, ADRs, plans, or other structured documents.
...
- One defect found is a class to sweep, not an instance to close — hand that to [variant-analysis procedure](variant-analysis.md).
...
- Reproducing a finding's root cause belongs to `observability`; writing the test it exposed to `test-engineering`; the delivery gate to `finishing-dev`.
