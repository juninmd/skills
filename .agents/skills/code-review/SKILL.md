...
4. Report findings first, ordered by severity. Each names `file:line`, trigger, actual consequence, evidence, and a concise correction. State when no actionable defects were found, then list scope and verification limits. Do not invent findings to fill a quota. A pre-merge pass before human review lists only problems that would block the merge, each with `file:line`, why it is wrong, and how to show it fails.
