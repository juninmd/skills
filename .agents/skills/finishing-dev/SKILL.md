...
5. Prepare a concrete PR title and body from the final diff and repository template: problem, resulting behavior, relevant design decisions, verification, and material limits. Attach [PR evidence](references/pr-evidence.md): UI/CLI screenshots and payloads of changed contracts. No raw logs, secrets, or conversational history. Complete this work even if publication authorization is missing.
...
- [ ] PR body prepared with required evidence; publication uses existing authorization.
