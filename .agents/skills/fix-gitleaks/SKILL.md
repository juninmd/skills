...
## When to Use
- The `gitleaks` job failed in GIT.
...
- [ ] Classify each finding before editing: real secret, placeholder, fixture, or false positive.
- [ ] Revoke or rotate real credentials before relying on ignore rules.
- [ ] Re-run gitleaks until the scan is clean and the remediation path is documented.
