---
name: deploy-ghcr-manual
description: |
  Build a container, push it to a registry and roll it out by hand as an escape hatch, when CI cannot build at all. Use when a billing block or a disabled token means a commit must reach the cluster and rerunning the build can never succeed.
license: MIT
---
