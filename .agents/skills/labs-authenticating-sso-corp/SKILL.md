---
name: labs-authenticating-sso-corp
description: Ability to implement authentication and authorization in internal collaborative applications using SSO Corp (Keycloak). Use for applications that manage employee logins. DO NOT use for M2M, B2B, or vendor integrations (use ID.Magalu instead).
metadata:
...
- When migrating an old application from legacy Keycloak Corp (idmagalu) to the new SSO Corp.
- **DO NOT** use for machine-to-machine (M2M), B2B, vendor, or seller integrations. For these cases, use ID.Magalu (Turia).
