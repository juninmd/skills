...
6. A slice with a schema change slices the data too — delegate the expand/backfill/contract shape to `data-engineering`.
...
- Sequencing the delivery is here; executing the pull requests belongs to `finishing-dev`, and running the full stage loop to `starting-dev`.
