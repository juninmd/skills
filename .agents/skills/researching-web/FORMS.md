# Web Researcher Formulários 📋
## 1. Research Plan Form (research_plan.md)

### Goal
Define the research objective and scope.

### Fields
- **Topic:** [Topic of the research]
- **Key Questions:**
    1. [Question 1]
    2. [Question 2]
    3. ...
- **Initial Search Queries:**
    1. [Query 1]
    2. [Query 2]
    3. ...

## 2. Research Report Form (research_report.md)

### Goal
Document the findings and answer the initial questions.

### Fields
- **Topic:** [Topic of the research]
- **Executive Summary:**
    [A brief overview of the key findings.]
- **Key Findings:**
    1. [Finding 1] (Source: [URL 1])
    2. [Finding 2] (Source: [URL 2])
    3. ...
- **Detailed Answers:**
    - **Question 1:**
        [Detailed answer based on findings.]
    - **Question 2:**
        [Detailed answer based on findings.]
    - ...
- **Sources Used:**
    - [URL 1] - [Title/Description]
    - [URL 2] - [Title/Description]
    - ...

