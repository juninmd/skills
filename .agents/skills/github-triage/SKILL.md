...
| `ready-for-agent` | State    | Fully specified, ready for an autonomous agent |
...
- [ ] Reconstruct the current issue state from labels, comments, and prior triage notes before asking new questions.
- [ ] Ask only concrete follow-up questions that unblock reproduction or classification.
- [ ] Leave the maintainer with a clear status, open questions, and next recommended action.
