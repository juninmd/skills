...
- `vitest` — Fast test execution, mocks, and coverage patterns for TypeScript projects.
- `diagnosing-networks` — HTTP, DNS, and connectivity diagnostics for external integrations.
...
- [ ] Keep each feature inside a Nest module with clear controller, service, and DTO boundaries.
- [ ] Enforce validation, auth, and error handling centrally instead of duplicating checks in handlers.
- [ ] Run the narrowest unit or e2e tests that cover the touched module before finishing.
