...
- Local changes: companion-file and `SKILL.md` links point at the renamed copies; mentions of sibling upstream skills link to the vendored copy or name the owning skill here. One macro heading says "compiler errors and warnings" instead of the upstream plural of "diagnostic", which matches a retired skill name. A generated `## Contents` list of the `##` headings follows the title in files over 100 lines (`write-swift.md`), per the house reference gate. No other edits.
