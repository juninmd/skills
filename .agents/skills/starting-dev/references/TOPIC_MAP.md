# starting-dev reference map

Open only the procedure matching the current subtask. These are files, not standalone skills.

| Reference | When to open |
|---|---|
| [AGENT-BRIEF.md](AGENT-BRIEF.md) | Prepare a bounded implementation brief when authorized issue tracking hands work to an agent |
| [agents-md.md](agents-md.md) | Generate repository instructions from observed tooling, ownership, and real commands |
| [codebase-mapping.md](codebase-mapping.md) | Trace entry points, imports, and one real request before planning changes |
| [docs-guidelines.md](docs-guidelines.md) | Select documentation types and evidence expectations during project setup |
| [docs-strategy.md](docs-strategy.md) | Choose audience and scope before reconstructing or extending repository docs |
| [entry-format.md](entry-format.md) | Format an explicitly requested durable learning entry with provenance and tags |
| [execution-guidelines.md](execution-guidelines.md) | Execute planned slices with checks, scope control, and progress evidence |
| [human-step-wizard.md](human-step-wizard.md) | Guide genuinely manual account or device setup without requesting credentials |
| [incremental-delivery.md](incremental-delivery.md) | Split a feature into independently verifiable vertical slices |
| [karpathy-methodology.md](karpathy-methodology.md) | Resolve ambiguity and keep implementation changes minimal during planning |
| [loop-state.md](loop-state.md) | Resume a sustained delivery task without losing gates, artifacts, or authority |
| [OUT-OF-SCOPE.md](OUT-OF-SCOPE.md) | Reject issue categories the requested triage workflow does not own |
| [phase-implement.md](phase-implement.md) | Execute domain slices and collect evidence before final independent review |
| [phase-plan.md](phase-plan.md) | Convert accepted direction into file-scoped steps and observable checks |
| [phase-prototype.md](phase-prototype.md) | Compare disposable variants when a real product direction is unresolved |
| [phase-research.md](phase-research.md) | Gather source-backed repository and API constraints before choosing an approach |
| [real-world-cases.md](real-world-cases.md) | Compare concrete examples when choosing a planning or review approach |
| [requirements-clarification.md](requirements-clarification.md) | Batch material unanswered product and acceptance questions before dependent work |
| [session-handoff.md](session-handoff.md) | Capture resumable state, evidence, and next actions for unfinished work |
| [session-learnings.md](session-learnings.md) | Record reusable lessons only when the user explicitly requests persistence |
| [spec-templates.md](spec-templates.md) | Choose the concrete PRD or implementation plan skeleton |
| [spec-workflow.md](spec-workflow.md) | Develop requirements into an actionable specification and verified slice plan |
| [to-issues-process.md](to-issues-process.md) | Convert accepted scope into independently verifiable issues when requested |
| [topic-map.md](topic-map.md) | Find GitHub Actions documentation sources relevant to workflow setup |
| [triage-logic.md](triage-logic.md) | Apply issue classification and ownership transitions during authorized triage |
| [triage-standards.md](triage-standards.md) | Check issue evidence and response quality before triage mutations |
| [triage-workflow.md](triage-workflow.md) | Execute requested issue triage with explicit action authority |
| [worktree-setup.md](worktree-setup.md) | Create an isolated worktree after resolving branch and task ownership |
| [worktree-standards.md](worktree-standards.md) | Verify worktree path, baseline, and isolation before development |
| [worktree-workflow.md](worktree-workflow.md) | Sequence worktree preparation and delivery handoff for parallel work |
