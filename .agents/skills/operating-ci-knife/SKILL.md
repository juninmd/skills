---
name: operating-ci-knife
description: The official CI/CD Swiss Army knife of MyProject for deployments, releases, validations, and security following the Senior standard.
metadata:
...
This skill encapsulates commands for the `ci-knife` tool, the gold standard for pipeline automation, release management, and security (SecurityAudit) at MyProject.
...
- To run security scans (SecurityAudit) and validate Quality Gates.
...
## Instructions
1.  **Prefer Tooling:** NEVER use manual scripts for deployments if `ci-knife` can perform the task. Standardization is essential for platform scale.
2.  **Security First:** The `security-scanner` command (SecurityAudit) is mandatory in `main` and `staging` pipelines. Analyze generated reports.
