...
description: Infrastructure, deployment, and dependency standards for MyProject (platform).
applyTo: '**/service-catalog.yaml, **/service-metadata.yaml, **/values.yaml, **/Dockerfile'
---

# Rule: platform Infrastructure

## Deployment and Infrastructure Examples
- **Deployment**: ArgoCD. Endpoint: `https://argocd-mke-operacoes-hml.ipet.sh`.
- **Repository**: GitLab. Endpoint: `https://gitlab.com`.
- **Environment Variables**: Google Secret Manager references in `values.yaml`.
  ...
  ```
- **GCP Access**: request via Papagali (platform Desenvolvedor).
...
- **Runtime**: local Docker and Kubernetes on platform Cloud.
- **CDN**: Azion, Akamai.
- **API Gateway**: Kong API.

## ContainerRegistry Info (Backstage Catalog)
- `service-metadata.yaml` is mandatory at repository root.
...
- `service-catalog.yaml` is mandatory at repository root.
...
3. **Hybrid Cloud Distinction**: clearly differentiate `GCP` and `MGC` (platform Cloud). Tooling, namespaces, and limitations are different.
